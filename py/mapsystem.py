class MapSystem:
    ...
    MAP_TEMP = Set(
        [Set([0 for _ in range(MAP_HEIGHT + 1)]) for _ in range(MAP_WIDTH + 1)]
    )
    ...
    def create_map() -> None:
        ...
        for y in brange(1, MAP_HEIGHT - 1):
            ...
            for x in brange(1, MAP_WIDTH - 1):
                if MAP_TEMP[x][y] > 0:
                    ...
                    temp = (
                        temp + min(MAP_TEMP[x][y + 1], 1) + min(MAP_TEMP[x][y - 1], 1)
                    )
                    if MAP_TEMP[x][y] < 255:
                        MAP_TEMP[x][y] = temp
                    match MAP_TEMP[x][y]:
                        ...
                        case 2:
                            if (
                                min(MAP_TEMP[x + 1][y], 1) + min(MAP_TEMP[x - 1][y], 1)
                                == 2
                            ):
                                room2_amount[zone] = room2_amount[zone] + 1
                            elif (
                                min(MAP_TEMP[x][y + 1], 1) + min(MAP_TEMP[x][y - 1], 1)
                                == 2
                            ):
                                room2_amount[zone] = room2_amount[zone] + 1
                            else:
        ...
        set_room(
            "room1archive", ROOM1, math.floor(0.5 * room1_amount[0]), min_pos, max_pos
        )
        ...
        set_room(
            "room2scps", ROOM2, math.floor(0.2 * (room2_amount[0])), min_pos, max_pos
        )
        set_room(
            "room2storage", ROOM2, math.floor(0.3 * (room2_amount[0])), min_pos, max_pos
        )
        set_room(
            "room2gw_b", ROOM2, math.floor(0.4 * (room2_amount[0])), min_pos, max_pos
        )
        set_room(
            "room2sl", ROOM2, math.floor(0.5 * (room2_amount[0])), min_pos, max_pos
        )
        set_room(
            "room012", ROOM2, math.floor(0.55 * (room2_amount[0])), min_pos, max_pos
        )
        set_room(
            "room2scps2", ROOM2, math.floor(0.6 * (room2_amount[0])), min_pos, max_pos
        )
        set_room(
            "room1123", ROOM2, math.floor(0.7 * (room2_amount[0])), min_pos, max_pos
        )
        set_room(
            "room2elevator",
            ROOM2,
        ...
        MAP_ROOM[ROOM3][
            math.floor(rng.rnd(0.2, 0.8) * (room3_amount[0]))
        ] = "room3storage"
        ...
        MAP_ROOM[ROOM2][
            room2_amount[0] + math.floor(0.1 * (room2_amount[1]))
        ] = "room2nuke"
        set_room(
            "room2tunnel",
            ROOM2,
        ...
        MAP_ROOM[ROOM3][
            room3_amount[0] + math.floor(0.3 * (room3_amount[1]))
        ] = "room513"
        MAP_ROOM[ROOM3][
            room3_amount[0] + math.floor(0.6 * (room3_amount[1]))
        ] = "room966"
                ...
                if MAP_TEMP[x][y] == 255:
                    ...
                    match temp:  # TRANSLATED COMMENT ;rooms in adjacent squares
                        case 1:
                            ...
                            r = create_room(
                                zone, ROOM1, x * 8, 0, y * 8, MAP_NAME[x][y]
                            )
                            if MAP_TEMP[x][y + 1] != 0:
                                r.angle = 180
                            elif MAP_TEMP[x - 1][y] != 0:
                        ...
                        case 2:
                            if MAP_TEMP[x - 1][y] > 0 and MAP_TEMP[x + 1][y] > 0:
                                if (
                                    MAP_ROOM_ID[ROOM2] < max_rooms
                                    and MAP_NAME[x][y] == ""
                                ):
                                    if MAP_ROOM[ROOM2][MAP_ROOM_ID[ROOM2]] != "":
                                        MAP_NAME[x][y] = MAP_ROOM[ROOM2][
                                            MAP_ROOM_ID[ROOM2]
                                        ]
                        ...
                        case 3:
                            ...
                            r = create_room(
                                zone, ROOM3, x * 8, 0, y * 8, MAP_NAME[x][y]
                            )
                            if MAP_TEMP[x][y - 1] == 0:
                                r.angle = 180
                            elif MAP_TEMP[x - 1][y] == 0:
                        ...
                        case 4:
                            ...
                            r = create_room(
                                zone, ROOM4, x * 8, 0, y * 8, MAP_NAME[x][y]
                            )
                            MAP_ROOM_ID[ROOM4] += 1
